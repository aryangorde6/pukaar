...
- **Proof it works:** [`verify.sh`](verify.sh) runs nineteen checks against the live stack. Each one asserts on rows and execution history, not on status codes. Last run 19/19. Unit tests and `terraform validate` run on every push: [![ci](https://github.com/aryangorde6/pukaar/actions/workflows/ci.yml/badge.svg)](https://github.com/aryangorde6/pukaar/actions/workflows/ci.yml). Every break during the build is in [`LEARNING-LOG.md`](LEARNING-LOG.md) with the commit that fixed it.
