...
- **Proof it works:** [`verify.sh`](verify.sh) runs sixteen checks against the live stack, each asserting on rows and execution history, never on a status code. Last run 16/16. The unit tests and `terraform validate` run on every push: [![ci](https://github.com/aryangorde6/pukaar/actions/workflows/ci.yml/badge.svg)](https://github.com/aryangorde6/pukaar/actions/workflows/ci.yml). Every break during the build is in [`LEARNING-LOG.md`](LEARNING-LOG.md) with the commit that fixed it.
...
- **Edge:** one Lambda Function URL, nine routes, no API Gateway, no login (the button is hers; the links are one-time, per incident, per person).
...
./verify.sh                                # sixteen live checks; reseeds before and after
