...
1. **Her screen** (`GET /`) is one button. It already says who will be told — *"Vaishali, Ravi and Anil will be told straight away"* — read from the same ranking the machine will use. It installs on her home screen as *Pukaar* (`/manifest.webmanifest`, two PNG icons served by the same function), so the button is an icon, not an address to type. It speaks her language: `?lang=mr` or `?lang=hi` once, at setup, and every word on her screen is Marathi — *मला मदत हवी आहे* — or Hindi; the phone remembers, and the one pill on the page switches between English and her language, never a menu. A language is twenty-two strings in [`web.py`](lambdas/web.py) and one person who speaks it and has read them; Marathi and Hindi are shipped because they were read. Only her page is translated; the people paged are younger, and their pages and emails stay English.
