...
2. **The press** (`POST /trigger`) starts one execution of `pukaar-escalation`, named after the incident, so a retried request cannot start the same incident twice. The button disables itself on press. A second press while an alert is running (a double tap, a reload, the app reopened) joins that alert instead of starting another, and her page reopened during one shows it. Two presses in the same instant are one alert: her row is taken with a conditional write before any machine starts.
...
| A second press during an alert does not start a second alert | `POST /trigger` returns the alert already running for her (open, widened, or claimed within the last half hour) instead of starting another; her page, reopened, shows that alert (check 15). Two presses in the same instant are one alert too: her row is taken with a condition before any machine starts, and the loser joins (check 21) |
| A late answer, or a cancel, after the last circle still tells everyone | once the machine has passed its last wait it cannot broadcast, so the web function does: a claim on a `FALLBACK` alert tells everyone reached who is coming, a cancel from there tells them it was a false alarm; a claim that lands in the second before `FinalFallback` writes is what `FinalFallback` sends instead of "nobody has gone"; a failure never overwrites a claim (check 21) |
...
### Decisions — what was chosen, against what, and what it costs

Each row is a choice made against a real alternative. The last column is the price paid for it. Numbered entries are in [`LEARNING-LOG.md`](LEARNING-LOG.md).

| Decision | Instead of | Because | The trade |
| --- | --- | --- | --- |
| **The escalation is a Step Functions Standard workflow.** | A loop in a server, or an Express workflow. | The product is mostly waiting: page three, wait a minute, widen. A server holding that loop has to stay up, and its crash is a silent alert. A Standard workflow parks at no cost per second, retries each step, and its execution history is where this README's numbers are read from. Express is priced by duration and capped at five minutes. | Every state transition is billed, including the ones the live checks spend. A real minute is a real minute, so the film compresses the timers. |
| **The wait is a task-token callback.** | A `Wait` state and a poll at the tier boundary. | With a timer, a claim or a cancel waits for the next tick: 15.4 s late on a 20 s test timer, up to a minute in production. The claim's own write (`updateItem.waitForTaskToken`) returns the parked token, and the machine wakes in about a second. | Past its last wait the machine has no token to wake, so a late claim or cancel is broadcast by the web function instead (entry 11, check 21). A stale token is `TaskTimedOut`, not `InvalidToken`, which had to be learned. |
| **Two answers, one winner, by one conditional write.** | Read the row, then write, in the web function. | Two people tapping in the same second is the normal case. A read-then-write lets both through and sends two people to her door. One `UpdateItem` with `status IN (OPEN, FALLBACK)` decides; the loser's page reads the winner's name from the row. | The condition has to hold everywhere the row is written: a claim landing in the second before `FinalFallback` could once turn a won alert into `FAILED` (entry 11); `record_failure` now refuses to overwrite a claim. |
| **The first circle is chosen by who answers, not who is nearest.** | The nearest three by distance, or a fixed order. | Inside a parallel `Map` there is no order, so the only decision is who is in the first minute at all. The nearest neighbour, who has never answered a 2 pm page, is paged a minute later; the son four kilometres away, who always answers, at once. | The ranking needs history. In the demo it is seeded, and a weekly check-in grows it between emergencies. One unanswered test page once erased five answered ones, until the counts got a two-page prior (entry 4). |
| **One Function URL and one-time links; no accounts.** | API Gateway with sign-in for her and for them. | Nobody on her list will create an account to be paged, and she should never see a login. The link is the permission: one per incident per person, hashed at rest, dead with the incident. Her page alone carries the key that can cancel or place her. Eleven routes, no auth layer to pay for. | Whoever holds a winning link sees her notes: logged, not gated by a second factor. A route that read by the incident id was, for one evening, a route that could act by it (entry 7). |
| **GET never writes.** | One-click claim links. | Mail scanners and link previews prefetch URLs; a claim on GET would have Gmail claiming alerts. Every link renders on GET and commits on POST. The button is the claim. | One more tap for the person going. |
| **The record is sealed under a customer-managed key, `Encrypt` direct, `subject_id` as context, no envelope.** | An AWS-managed key, or envelope encryption with a crypto library. | AWS-managed keys cannot be called from your own code. The record is 251 bytes, under the 4 KB direct limit, and the standard library has no AES, so an envelope would mean a dependency. The context binds the ciphertext to one person: another id fails to decrypt (check 9), and only the web role, with a context, may try. | The $1/month key is the largest line in the bill. A record over 4 KB would need the envelope after all. |
| **Email always; Telegram for whoever has started her bot.** | SMS, WhatsApp, Web Push. | Sender-ID SMS to India needs a DLT registration built on company tax registrations an individual cannot supply, at ₹0.20 or more a message. WhatsApp is gated by Meta's review of the sender and of every template. Web Push needs a compiled dependency the packaging rules out. SES is about ₹0.013 a message, Telegram nothing, and the same link goes on both, so a person is reached if either took it. | Dispatch is not delivery: where Gmail files the page cannot be seen from this side. A bot cannot message first, so the son taps *Start* once at setup, which is also the consent step. |
| **A circle that reached nobody fails the execution.** | Carry on into the wait and let the next tier's timer sort it out. | Waiting sixty seconds for a claim that cannot arrive is the system quietly doing nothing. Zero reached goes to `RecordFailure`, the execution fails, and an EventBridge rule on that ending mails the operator through SNS; her button's own errors and bounced pages reach the same topic. | The operator is mailed on every failure, including the ones the live checks provoke on purpose, four a run. A configuration set made the sender's default once denied every send (entry 10). |
| **Proof runs against the live stack and asserts on rows and execution history.** | Unit tests against a mocked AWS, or checks on HTTP status codes. | A 200 says a page rendered, not that the row changed or that three sends carried one timestamp. The checks send real pages, read the tables and the execution history, and reseed before and after. | About four minutes and real transitions a run. A check that passes on the wrong thing is worse than none (entries 2, 4, 6), so the checks are themselves under test. |
| **One zip, standard library and boto3 only, no layers.** | A dependency layer. | Nothing to build, pin or rebuild for arm64; `terraform apply` is the whole deploy, and the eight functions ship from one archive. | No AES, so no envelope encryption; no Web Push. No template engine either: the pages and the mails are Python strings. |
...
11. **A second reader found five things twenty checks by one author could not.** With the code frozen, the repo went to a reviewer that had never seen it (Cursor, read-only, told to report file and line with a way to confirm). A claim or a cancel after the last circle told nobody, because the machine was past its last wait and `wake()` handed back a stale token; a claim in the second before `FinalFallback` wrote could turn a won alert into `FAILED`; two presses in the same instant made two alerts; a body that was not JSON on `/cancel` was an unhandled error, and since the afternoon an unhandled error pages the operator. Every one was on a path the checks never walked: I had tested the paths I imagined, and I had imagined the ones I built. Check 21 walks them now. Fix [`09e67c2`](https://github.com/aryangorde6/pukaar/commit/09e67c2), entry 21:55.

What the eleven have in common (seven were a word I trusted, three were checks that checked the wrong thing, one was statistics) is written at the end of the log, with what I would carry to the next build.
...
AI tools used in this build: Claude Code (Anthropic), as a coding assistant; Cursor, for one read-only review of the frozen code on the last evening (learning-log entry 11).
