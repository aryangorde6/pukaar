...
def location(event):
    """Her phone's position, sent by her page after a press. Written on the incident while it
    is live; the pages the responders open read it from there. Nothing else trusts it."""
    try:
        body = json.loads(event.get("body") or "{}")
        incident_id = str(body["incident_id"])[:64]
        lat, lon, acc = float(body["lat"]), float(body["lon"]), int(float(body.get("accuracy_m", 0)))
    ...
    try:
        ddb.update_item(TableName=INCIDENTS, Key={"incident_id": {"S": incident_id}},
                        ...
                        ConditionExpression="subject_id = :s AND #st IN (:open, :fallback, :claimed)",
                        ExpressionAttributeNames={"#l": "location", "#st": "status"},
                        ExpressionAttributeValues={":l": {"M": {"lat": {"N": f"{lat:.5f}"}, "lon": {"N": f"{lon:.5f}"},
                                                          "accuracy_m": {"N": str(max(0, acc))}, "at": {"N": str(now)}}},
                                                   ":s": {"S": SUBJECT_ID}, ":open": {"S": "OPEN"},
                                                   ":fallback": {"S": "FALLBACK"}, ":claimed": {"S": "CLAIMED"}})
    except ClientError as e:
        if e.response["Error"]["Code"] != "ConditionalCheckFailedException":
            raise
        return jsonr(409, {"error": "no live alert with that id"})
    print(json.dumps({"component": "web", "event": "location_reported", "incident_id": incident_id, "accuracy_m": acc}))
    return jsonr(200, {"incident_id": incident_id, "at": fmt_time(now)})
...
def status(incident_id):
    ...
    if "releases" in item:  # the one who was coming stepped back; her page says so
        out["stepped_back"] = item["releases"]["L"][-1]["M"]["name"]["S"]
    if out["status"] in ("OPEN", "FALLBACK"):  # who has been told by now: her screen follows the widening
...
def where_line(incident):
    """Her phone's last position on this alert, if her page sent one; else nothing."""
    loc = incident.get("location", {}).get("M")
    if not loc:
        return ""
    lat, lon = float(loc["lat"]["N"]), float(loc["lon"]["N"])
    acc, at = int(loc["accuracy_m"]["N"]), fmt_time(loc["at"]["N"])
    return (f'<p class="address"><strong>Her phone, at {at}:</strong> within {acc} m of '
            f'<a href="https://maps.google.com/?q={lat:.5f},{lon:.5f}" target="_blank" rel="noopener">this spot</a>.</p>')
...
STRINGS = {
    "en": {
        ...
        "foot": "Not working? Call", "and": " and ", "location": "Let helpers see where you are", "stepped_back": "{name} can’t come after all.", "noone_h1": "No one has answered yet", "call_now": "Call 112 now", "name": "English",
    },
    "mr": {
        ...
        "foot": "चालत नाही? फोन करा", "and": " आणि ", "location": "मदत करणाऱ्यांना तुम्ही कुठे आहात ते दिसू द्या", "stepped_back": "{name} आता येऊ शकणार नाहीत.", "noone_h1": "अजून कोणीही उत्तर दिलेले नाही", "call_now": "आत्ताच 112 ला फोन करा", "name": "मराठी",
    },
    "hi": {
        ...
        "foot": "काम नहीं कर रहा? फ़ोन करें", "and": " और ", "location": "मदद करने वालों को दिखने दें कि आप कहाँ हैं", "stepped_back": "{name} अब नहीं आ पाएँगे.", "noone_h1": "अभी तक किसी ने जवाब नहीं दिया", "call_now": "अभी 112 पर फ़ोन करें", "name": "हिन्दी",
    },
...
<main id="noone" hidden>
  <div class="card card-emergency"><h1 data-i18n="noone_h1">No one has answered yet</h1>
  <p id="noone-names"></p></div>
  <a class="btn btn-emergency" href="tel:112" data-i18n="call_now">Call 112 now</a>
  <button class="btn btn-secondary" id="cancel3" data-i18n="cancel">Cancel — I’m OK</button>
</main>
...
(function () {
  ...
  var setLang = function (l) {
    ...
    document.getElementById("noone-names").innerHTML = fill("sent_names", "names", joinNames(names));
  ...
  var show = function (id) {
    if (id === shown) return;
    shown = id;
    ["idle", "sent", "failed", "coming", "noone", "cancelled"].forEach(function (s) { document.getElementById(s).hidden = (s !== id); });
  ...
  var check = function () {
    ...
    fetch("/status/" + incident).then(function (r) { return r.json(); }).then(function (s) {
      ...
      else {  // still open: her screen follows the alert - who has been told by now, and whether anyone answered
        if (s.told) {
          if (s.stepped_back) names = s.told;  // the one who was coming is off the list
          else names = names.concat(s.told.filter(function (n) { return names.indexOf(n) < 0; }));  // the circle widened
          var filled = fill("sent_names", "names", joinNames(names));
          document.getElementById("sent-names").innerHTML = filled;
  ...
  // Where she is now, for the people coming: asked once at setup (the small line under the
  // button, gone once answered either way), then sent with every press, after the page is
  // already paged - the alert never waits on the phone's GPS.
  ...
  var allow = document.getElementById("allow-location");
  try {
    navigator.permissions.query({ name: "geolocation" }).then(function (st) {
      allow.hidden = st.state !== "prompt";
      st.onchange = function () { allow.hidden = st.state !== "prompt"; };
    });
  } catch (e) {}
  allow.addEventListener("click", function () {
    try { navigator.geolocation.getCurrentPosition(function () { allow.hidden = true; }, function () { allow.hidden = true; }, { timeout: 15000 }); } catch (e) {}
  });
  var press = function () {
